...
aliases:
  - "switch"
  - Switch
created: 2026-04-19T00:09:53
modified: 2026-04-19T00:17:13
description: "Switch 安装 Atmosphère 手册"
source: "https://github.com/acgotaku/switch"
tags:
  - "github/star"
---
...
![](https://img.shields.io/github/stars/acgotaku/switch?style=for-the-badge&label=stars) ![](https://img.shields.io/github/repo-size/acgotaku/switch?style=for-the-badge&label=size) ![](https://img.shields.io/github/created-at/acgotaku/switch?style=for-the-badge&label=since)

[![](https://github-stats-extended.vercel.app/api/pin/?username=acgotaku&repo=switch&bg_color=00000000)](https://github.com/acgotaku/switch)

本手册基于自己的安装实践，仅供参考。

## 1\. 准备工作

1. 可以软破的 Switch 机器，Switch 破解分为软件破解和硬件破解。硬件破解技术难度高，不在讨论范围内。软件破解需要 2018 年 6 月之前出厂的机器，具体能破解的序列号请参考下图：

![](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/code.png)
...
2. [hekate](https://github.com/CTCaer/hekate/releases/latest) Switch 引导文件
	...
	把 RCM 注入器插入电脑，会识别为 U 盘，打开 `\ATMOSPHERE_HEKATE` 文件夹，把刚才的 `hekate_ctcaer_xxx.bin` 复制为 payload.bin，然后拔掉。 最后把里面的 `bootloader` 文件夹拷贝到 SD 卡根目录。
3. [Signature Patches](https://sigmapatches.su/) 这个补丁可以支持安装非官方签名的游戏文件
	下载 `SigPatches` 并解压到 SD 卡根目录。[sigpatch-updater](https://github.com/ITotalJustice/sigpatch-updater) 可以直接从 Switch 下载最新的补丁。 Atmosphère 系统有两个引导方案，默认的是 fss0 引导，如果想使用这个补丁必须使用 fusee 引导。
4. [Lockpick\_RCM](https://github.com/shchmue/Lockpick_RCM/releases/latest) 这个软件可以获取本机密钥，在处理 Switch 文件或者变砖修复的时候会用到，如果担心出问题最好提前备份。
...
1. [/bootloader/hekate\_ipl.ini](/acgotaku/switch/blob/master/sdcard/bootloader/hekate_ipl.ini) 这是引导的配置文件，可以类比为 GRUB2 的启动项描述文件。
	启动项里面有图标描述文件，位置在 [/bootloader/res](/acgotaku/switch/blob/master/sdcard/bootloader/res) 也一并复制过去。
2. [/exosphere.ini](/acgotaku/switch/blob/master/sdcard/exosphere.ini) 这是大气层的配置文件，这个配置主要是为了隐藏主机的序列号防止被 ban
3. [/atmosphere/hosts/emummc.txt](/acgotaku/switch/blob/master/sdcard/atmosphere/hosts/emummc.txt) 这是 hosts，这个配置主要是为了屏蔽任天堂服务器
...
1. 点击 Tools
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/backup1.png)
2. 点击 Backup eMMC
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/backup2.png)
3. 点击 eMMC BOOT0 & BOOT1
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/backup3.png)
4. 等待完成，点击 Close
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/backup4.png)
5. 接下来点击 eMMC RAW GPP
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/backup5.png)
6. 等待完成，点击 Close -> Home -> Power Off
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/backup6.png)
...
系统完全安装在 SD 卡内，可以保证不污染 Switch 内部存储，是比较推荐的安装方式。 目前安装方式有两种，`SD File` 和 `SD Partition`， 推荐后者，因为后者的读取速度更快。

1. 打开 Hekate， 选择 Tools， 然后点击 Partition SD Card
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/Step1.png)
2. 请保证 SD 卡的使用容量在 1 GB 以下，因为 1GB 以下会显示 `Your SD Card files will be backed up automatically!`。 1GB 以下会自动备份和恢复数据，否则需要自行备份数据。
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/Step2.png)
3. 调整 emuMMC (RAW) 为 29 GiB，界面上会显示 `29 full`
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/Step3.png)
4. 选择 `Next Step`，点击 `Start` 然后等待完成。
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/Step4.png)
5. 返回 Home，点击 `emuMMC`
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/Step5.png)
6. 选择 `Create emuMMC`
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/Step6.png)
7. 选择 `SD Partition`
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/Step7.png)
8. 选择 `Part 1`
	![)](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/Step8.png)
9. 等待完成，点击 Close -> Home -> Launch ，选择 `Atmosphere Fusee CFW` 启动。然后就开始进入系统了。
10. 进入系统，查看系统设置，如果显示当前版本包含字母 `E` 说明成功进入虚拟系统了。
	![](https://raw.githubusercontent.com/acgotaku/switch/refs/heads/master/images/SystemSettings.jpeg)
...
Tinfoil 支持 [网络安装](https://blawar.github.io/tinfoil/network/)，这里推荐使用 `Samba` 方式来安装，PC 开启 SMB 共享之后， 使用 Switch 进行访问，既能解决 FAT32 不支持 4GB 以上文件，游戏无法复制到 SD 卡上的问题，又能避免每次安装都要关机插拔 SD 卡到电脑复制游戏。 不过联网之前记得使用 [90DNS](https://gitlab.com/a/90dns) 屏蔽任天堂服务器，避免机器被 ban。
...
1. 请先升级 Atmosphère，Hekate，Signature Patches，升级方法很简单下载最新版本到 SD 卡上，并且更新注入器的 `payload.bin`。 需要先删除 `atmosphere` 和 `bootloader` 文件夹再复制，强行覆盖的话不能保证成功启动。
2. 到 [Switch Firmwares](https://darthsternie.net/switch-firmwares/) 去下载最新固件。并解压到 SD 卡根目录。如果 Daybreak 校验失败，是 MacOS 添加隐藏文件的问题，可以通过 `dot_clean` 命令来清除隐藏文件。
	dot\_clean /Volumes/SWITCH\\ SD/Firmware\\ 17.0.1/
...
## 参考资料 [NH Switch Guide](https://nh-server.github.io/switch-guide/)
